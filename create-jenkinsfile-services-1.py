...
pipeline {{
  ...
  stages {{
    ...
    stage('Verify Image in ECR') {{
      steps {{
        script {{
          def result = sh(
            script: "aws ecr describe-images --repository-name petclinic/${{SERVICE_NAME}} --image-ids imageTag=${{IMAGE_TAG}} --region $AWS_REGION",
            returnStatus: true
          )
          if (result != 0) {{
            error "Image not found in ECR: petclinic/${{params.SERVICE_NAME}}:${{IMAGE_TAG}}"
          }} else {{
  ...
  post {{
    success {{
      echo "Pushed ${{SERVICE_NAME}} successfully to ECR"
    }}
    failure {{
