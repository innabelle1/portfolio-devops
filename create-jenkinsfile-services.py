...
# Updated Jenkinsfile template for re-tagging and pushing existing DockerHub images to ECR
...
pipeline {{
  ...
  parameters {{
    ...
    string(name: 'DOCKERHUB_REPO', defaultValue: 'innabelle1/spring-petclinic-{name}', description: 'DockerHub repo name')
  }}
  ...
  post {{
    failure {{
      echo "Failed to sync image for ${{params.SERVICE_NAME}}"
    }}
    success {{
      echo "Image ${{params.SERVICE_NAME}}:$IMAGE_TAG successfully pushed to ECR"
    }}
  }}
}}
"""

# Generate Jenkinsfile for each service
for name in services:
    folder = Path(f"./spring-petclinic-{name}")
    folder.mkdir(exist_ok=True)  # Ensure folder exists
